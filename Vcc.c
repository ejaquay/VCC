...
unsigned __stdcall EmuLoop(void *Dummy)
{
	...
	while (true) 
	{
		...
		char ttbuff[256];
		snprintf(ttbuff,sizeof(ttbuff),"KB:%d | Skip:%2.2i | FPS:%3.0f | %s @ %2.2fMhz| %s",
				GetKeyboardLayout(), EmuState.FrameSkip, FPS, CpuName, 
				EmuState.CPUCurrentSpeed, EmuState.StatusLine);
